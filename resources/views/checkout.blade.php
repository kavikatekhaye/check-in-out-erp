...
<div class="container">
    ...
    <div class="row">
        <div class="col-md-5">
            <h2 style="margin-top:30px">Check-Out System</h2>
            @if (Session::has('success'))
                <div class="alert alert-success">{{ Session::get('success') }}</div>
            @endif

            <form action="{{ route('employee.checkout.store', $user['id']) }}" method="POST" autocomplete="off">
                @csrf
                ...
                <div class="mb-3 col-md-12">
                    ...
                    <select class="form-control" name="user_id">
                        ...
                        @foreach ($data as $u)
                            <option {{ $u['id'] == $user['user_id'] ? 'selected' : '' }} value="{{ $u['id'] }}">
                                {{ $u['name'] }}</option>
                        @endforeach
                    </select>
